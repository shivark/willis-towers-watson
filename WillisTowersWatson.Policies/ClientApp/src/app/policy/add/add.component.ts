...
export class AddEditComponent implements OnInit, OnDestroy {
  ...
  saveClicked(): void {
    ...
    console.log("FORM", this.policyForm.value);
  ...
  private displayPolicy() {
    this.routSubscription = this.route.paramMap
      .pipe(
          ...
          this.policyService.getById(id).pipe(
            ...
            tap(p =>
              this.policyForm.patchValue({
                name: p.policyHolder.name,
                age: p.policyHolder.age
              })
            )
          )
        ),
  ...
  private setPageTitle() {
    this.titleService.setTitle(
      this.policy ? PAGE_TITLES.EDIT : PAGE_TITLES.ADD
    );
  }
